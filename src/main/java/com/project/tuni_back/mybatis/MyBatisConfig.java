//package com.project.tuni_back.mybatis;
//
...
//@MapperScan("com.project.tuni_back.mapper")  // Mapper를 스캔할 패키지
...
import javax.sql.DataSource; // DataSource import
...
@Configuration
@RequiredArgsConstructor
@MapperScan("com.project.tuni_back.mapper")
public class MyBatisConfig {

    private final ApplicationContext applicationContext;
    private final DataSource dataSource; // ◀️ Spring Boot가 yml로 자동 생성한 DataSource를 바로 주입받음

    @Bean
    public SqlSessionFactory sqlSessionFactory() throws Exception { // throws Exception으로 변경
        SqlSessionFactoryBean sfb = new SqlSessionFactoryBean();
        sfb.setDataSource(dataSource); // 주입받은 dataSource를 바로 사용

        // SQL을 작성할 xml 경로 설정
        sfb.setMapperLocations(applicationContext.getResources("classpath*:/mapper/*.xml"));
        
        // MyBatis의 다른 설정 (예: CamelCase)
        org.apache.ibatis.session.Configuration mybatisConfig = new org.apache.ibatis.session.Configuration();
        mybatisConfig.setMapUnderscoreToCamelCase(true);
        sfb.setConfiguration(mybatisConfig);
